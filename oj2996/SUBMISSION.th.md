...
```text
โจทย์ต้องการให้เราทำการนำ input ที่เป็น string มาสลับ index จากหน้าไปหลัง หลังไปหน้า และทำทุกตัวอักษรให้เป็นตัวพิมพ์เล็ก และในโจทย์นี้ยังไม่มี constraints ในการเขียนโค้ด
```
...
```
Step 1: เริ่มจากการทำความเข้าใจโจทย์ว่า input คืออะไร โจทย์ต้องการให้เราทำอะไรกับ input
Step 2: หลังจากเข้าใจโจทย์เสร็จ ต้องลองคิด logic ครับว่า เราสามารถใช้ built in function หรืออะไรต่างๆ ได้บ้างใน python เพื่อให้การทำโจทย์นี้ง่ายขึ้น ซึ่งในที่นี้ก็คือการใช้ index slicing ในstring และการใช้ function lower() ครับ
Step 3: สุดท้ายก็ลงมือทำเขียนโค้ดตามที่เราได้คิดไว้
```
...
```text
สุดท้ายที่ผมคิดได้ก็คือการใช้ index slicing โดยจะไม่เขียนอะไรในส่วนของ start stop แต่จะใส่ -1 ตรง step เพื่อให้มันไปทีละตัวอักษรแต่ย้อนหลังครับ กับการใช้ lower() เพื่อให้มันได้ผลลัพธ์ตามที่โจทย์อยากได้ครับ
```
...
```text
เนื่องจากโจทย์เป็นเกี่ยวกับการสลับตัวอักษร ผมจึงเลือกชื่อตัวเองมาใช้ก่อนเป็นอันดับแรกเพราะว่าเป็นสิ่งแรกที่ผมนึกได้ครับ
```
...
```text
ด้วยความที่โจทย์นี้มันเป็นเกี่ยวกับการสลับ index string และการทำให้ตัวอักษรเป็นตัวพิมพ์เล็กผมจึงลองทำให้เป็นตัวพิมพ์ใหญ่ดูครับ
...
```text
เนื่องจากใน 2 testcase ที่ผ่านมายังคงมี characters ที่น้อยผมจึงลองใช้ชื่อจริงตัวเองเพื่อลอง input ที่มีค่า index เยอะครับ
```

Input:

```text
NATTAKITCHANTARAAREE
```

Expected output:

```text
eeraaratnahctikattan
```
