...
```text
โจทย์กำหนดให้ผมรับค่าตำแหน่ง (เลขจำนวนเต็มบวก) โดยจะต้องเขียนโปรแกรมเพื่อดูว่าจะต้องเดินน้อยที่สุดกี่ครั้งถึงจะเดินทุบกำแพงจากเลขที่กำหนดไปเลข 1 ได้สำเร็จ โดยห้ามออกจากเขตสามเหลี่ยม

Input:
รับค่าจำนวนเต็มบวกไม่เกิน 100000
Output:
บอกจำนวนการเดินทุบกำแพงที่น้อยที่สุดเพื่อถึงเลข 1
Constraints:
input เป็นจำนวนเต็ม ห้ามเป็นเลข 0 หรือติดลบ 
...
```text
Step 1: รับ input จำนวนเต็ม
Step 2: ดูว่าเลขนี้อยู่ชั้นอะไรของสามเหลี่ยม
Step 3: ดูว่าเลข input นั้นเป็นเลขคู่หรือคี่ และชั้นเป็นเลขคู่หรือคี่
Step 4: หากเป็นคี่ทั้งคู่ หรือ คู่ทั้งคู่ จะทำการลบชั้นด้วย 1 และคูณ 2
Step 5: เงื่อนไขที่เหลือ ทำการลบชั้นด้วย 2 มาคูณ 2 และบวก 1
```
...
```text
วิธีสุดท้ายของผมคือการนำเลขที่ input ได้มา math.sqrt และ math.floor เพื่อทราบชั้นว่าเลขที่อยู่ชั้นอะไร จากนั้นนำชั้นและเลข input ว่าเป็นเลขคี่หรือคู่ทั้งคู่ไหม หากใช่จะ print (ชั้น - 1) * 2 หากไม่ใช่จะทำการ print ((ชั้น - 2) * 2) + 1
```
...
```text
เป็นเลขแรกของชั้นครับ จะเป็นค่าทศนิยมที่น้อยที่สุดของชั้นก่อนที่จะ math.ceil
```
...
```text
เป็นเลขที่อยู่ติดกับ 26 ซึ่งจะต้องมีการเดินน้อยกว่าค่า test แรก 1 ก้าวเนื่องจากเป็นเลขคี่ที่อยู่ชั้นเลขคู่
```
...
```text
36 เป็นเลขยกกำลังที่อยู่ขวาสุดของชั้น 3 เหลี่ยมซึ่งหลังจาก math.sqrt จะได้เป็นค่าจำนวนเต็ม
```
